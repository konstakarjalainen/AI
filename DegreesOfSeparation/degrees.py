...
def shortest_path(source, target):
    ...
    trg_neighbors = list(neighbors_for_person(target))
    trg_neighbors_names = [t[1] for t in trg_neighbors]
    ...
    while not frontier.empty():
        ...
        for movie, actor in neighbors:
            if actor not in explored and not frontier.contains_state(actor):
                new_node = Node(state=actor, parent=current_node, action=movie)
                if new_node.state == target:
                    # toc = time.perf_counter()
                    # print(f"Found the path in {toc - tic:0.4f} seconds")
                    this_path = []
                    node = new_node
                    while node != source_node:
                        ...
                        node = node.parent
                    return this_path
                elif new_node.state in trg_neighbors_names:
                    ind = trg_neighbors_names.index(new_node.state)
                    last_movie = trg_neighbors[ind][0]
                    this_path = []
                    node = new_node
                    this_path.insert(0, (last_movie, target))
                    while node != source_node:
                        this_path.insert(0, (node.action, node.state))
                        node = node.parent
                    return this_path
