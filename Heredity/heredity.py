...
def child_probability(genes, parent_genes):
    if genes == 0:
        return parent_genes[0] * parent_genes[1]
    elif genes == 1:
        return (1 - parent_genes[0]) * parent_genes[1] + parent_genes[0] * (1 - parent_genes[1])
    else:
        return (1 - parent_genes[0]) * (1 - parent_genes[1])
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in parents:
        ...
        if person in one_gene:
            genes = 1
            parent_genes.append(0.5)
        elif person in two_genes:
            genes = 2
            parent_genes.append(PROBS['mutation'])
        else:
            genes = 0
            parent_genes.append(1 - PROBS['mutation'])

        joint_prob *= PROBS['gene'][genes] * PROBS['trait'][genes][is_trait]
    ...
    for person in children:
        ...
        joint_prob *= child_probability(genes, parent_genes) * PROBS['trait'][genes][is_trait]
